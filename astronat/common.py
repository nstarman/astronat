...
@quantity_io()
def angular_distance_on_sky(
    lon1: DEG, lat1: DEG, lon2: DEG, lat2: DEG
) -> T.Any:
...
def apparent_to_absolute_magnitude(m: MAG, d_L: PC, **kw) -> T.Any:
...
@quantity_io(M=MAG, d_L="distance")
def absolute_to_apparent_magnitude(M: MAG, d_L: PC, **kw) -> T.Any:
...
def distanceModulus_magnitude(d: PC, A=0 * MAG, obs=True, **kw) -> T.Any:
...
def distanceModulus_distance(DM: MAG, **kw) -> T.Any:
    r"""Distance Modulus.
...
def distanceModulus(arg, A=0 * MAG, obs=True, **kw) -> T.Any:
    r"""Distance Modulus.
...
def parallax_angle(d: PC, **kw) -> T.Any:
...
@quantity_io(p="angle")
def parallax_distance(p: DEG, **kw) -> T.Any:
...
def max_angular_separation(doff, dto, **kw) -> T.Any:
